...
import type { GroupBaseline } from "@intune-baseline/shared";
import { api } from "./api.ts";

export function BaselinePanel({ groupId, onClose }: { groupId: string; onClose: () => void }) {
  const [baseline, setBaseline] = useState<GroupBaseline | null>(null);
  ...
  useEffect(() => {
    setBaseline(null);
    setError(null);
    api.baseline(groupId).then(setBaseline).catch((e) => setError(e.message));
  }, [groupId]);

  const conflictIds = new Set(baseline?.conflicts.map((c) => c.settingId) ?? []);
  const rows =
    baseline?.settings.filter(
      (s) =>
        !filter ||
  ...
  return (
    <div className="fixed inset-y-0 right-0 z-20 flex w-full max-w-xl flex-col border-l border-ink-700 bg-ink-900 shadow-2xl">
      <div className="flex items-start justify-between border-b border-ink-700 p-4">
        <div>
          <div className="text-xs uppercase tracking-wide text-slate-400">Merged baseline</div>
          <h2 className="text-lg font-semibold text-slate-100">{baseline?.group.displayName ?? "Loading…"}</h2>
          {baseline && (
            <div className="mt-1 text-xs text-slate-400">
              {baseline.policies.length} policies · {baseline.settings.length} settings ·{" "}
              <span className={baseline.conflicts.length ? "text-amber-400" : "text-emerald-400"}>
                {baseline.conflicts.length} conflicts
              </span>
            </div>
          )}
        </div>
                ...
                {rows.map((s) => (
                  <tr
                    ...
                    className={`border-t border-ink-800 ${
                      conflictIds.has(s.settingId) ? "bg-amber-500/10" : ""
                    }`}
                  >
