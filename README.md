...
- 🏷️ **Autopilot Group Tag aware** — flag the endpoint as an Autopilot device and type its Group Tag; the tool auto-selects the dynamic groups whose **Group Tag rule** matches it — the `[OrderID]` clauses in `device.devicePhysicalIds` (`-eq` / `-startsWith`, including several such clauses joined by `or`/`and`). It reads **only** those Group Tag (and `[ZTDId]` Autopilot) clauses — *not* other rule conditions such as device model or OS — so groups keyed on those are left for you to select manually (see the limitations note below). The "Autopilot device" toggle also links the default Autopilot-joined groups (bare `[ZTDId]` rule).
...
- **Dynamic membership is evaluated best-effort, not queried.** The app does not ask Graph whether a specific device/user is in a group; it reasons over the membership *rules*:
  - **Group Tag matching** looks at **only** a group's `[OrderID]` (Group Tag) clauses in `device.devicePhysicalIds` (`-eq` / `-startsWith`, with one level of `or` / `and` *across those clauses*). Any *other* condition in the rule — device model, OS version, enrollment profile, etc. — is ignored by the auto-selection.
  - **Rule implication** and the **Autopilot-joined** link (`[ZTDId]`) are heuristics over the same `devicePhysicalIds` clause shapes.
  - **So combined / nested rules aren't fully evaluated.** If a group's rule combines a Group Tag clause with something else (e.g. `... or device.deviceModel -eq "Cloud PC"`), nests parentheses, or uses operators beyond `-eq`/`-startsWith`, those extra parts are ignored — the group may be **missed** (when its membership actually comes from the ignored condition) or **over-matched**. This is intentional for the 0.5 beta; a full rule-expression evaluator is planned for v1 — see [ROADMAP.md](ROADMAP.md).
